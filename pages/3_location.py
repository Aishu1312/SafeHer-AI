...
st.title(f"📍 {_('Live Location Tracking')}")
st.caption(_("Share your exact location with trusted contacts"))
st.markdown("---")
...
def get_address(lat, lon):
    try:
        ...
        return location.address if location else _("Address not found")
    except Exception as e:
        return f"{_('Could not fetch address')}: {e}"

st.markdown(f"### {_('Get Your Current Location')}")
...
with col1:
    ...
    if loc and loc.get('latitude') and loc.get('longitude'):
        ...
        st.success(f"{_('Location Found')}: {lat:.6f}, {lon:.6f}")
        
        # Fetch Address
        with st.spinner(_("Fetching address...")):
            address = get_address(lat, lon)
            st.session_state.user['location']['address'] = address
            st.markdown(f"**📍 {_('Current Address')}:** {address}")
        
        # Map Display
        m = folium.Map(location=[lat, lon], zoom_start=15, tiles="CartoDB dark_matter")
        folium.Marker(
            [lat, lon],
            popup=_("You are here"),
        ...
        st.info(_("Click the button above to allow GPS access and find your location."))
...
with col2:
    st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
    st.subheader(f"🔗 {_('Share Location')}")
    ...
    if st.session_state.user['location']['lat']:
        ...
        message = f"{_('Please track my live location. I am at')}: {maps_link}"
        ...
        st.warning(_("Fetch your location first to enable sharing."))
